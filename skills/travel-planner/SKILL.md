...
This skill transforms Codex into a comprehensive travel planning assistant that maintains your travel preferences and generates detailed, personalized trip plans including itineraries, budget breakdowns, packing lists, and cultural guidelines for any destination.
...
```bash
python scripts/travel_db.py is_initialized
```
...
2. **Best Time to Visit** - 天气、季节、节庆活动
3. **Safety Information** - 安全提示、常见骗局
4. **Cultural Norms** - 当地习俗和禁忌（参考 `references/cultural_etiquette.md`）
5. **Local Transportation** - 地铁、公交、打车软件
6. **Top Attractions** - 必去景点、开放时间、门票价格
7. **Food Recommendations** - 当地特色、推荐餐厅
8. **Neighborhoods** - 住宿区域推荐
9. **Day Trip Options** - 周边一日游
10. **Practical Info** - 货币、小费、电源插座、语言

### Step 6: Generate Detailed Travel Plan
...
Codex: [Checks preferences - finds initialized]
...
Codex: [Creates trip in database, performs web research]
...
*路径示例：*
- macOS/Linux: `./codex-data/travel_planner/`
- Windows: `./codex-data/travel_planner\`
...
# View data
python scripts/travel_db.py get_preferences
python scripts/travel_db.py get_trips current
python scripts/travel_db.py stats

# Generate plan
python scripts/plan_generator.py --trip-id <id> --output plan.json

# Export backup
python scripts/travel_db.py export > backup.json
```
